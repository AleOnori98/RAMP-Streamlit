...
def run_single_archetype(excel_path: Path, num_days: int) -> Dict:
    ...
    # - minute-long: 525600 rows
    # - hourly-long: 8760 rows (optional, if you also want hourly per-category)
    EXPORT_MINUTE_LONG = True
    EXPORT_HOURLY_LONG = False  # set True if you also want per-category hourly exports

    per_user_csv: Dict[str, str] = {}

    for cat, year_mat in per_cat_year.items():
        # year_mat is (365, 1440)
        if EXPORT_MINUTE_LONG:
            df_long = matrix_to_long_series(
                year_mat, freq="T", year=2020, col_name="power_W", add_datetime_index=True
            )
            path = PM.outputs_dir / f"profile_{_safe_name(cat)}_minute_long.csv"
            save_dataframe_csv(df_long, path)
            per_user_csv[cat] = str(path.resolve())
        else:
            # fallback to old wide
            df = pd.DataFrame(year_mat)
            path = PM.outputs_dir / f"profile_{_safe_name(cat)}.csv"
            save_dataframe_csv(df, path)
            per_user_csv[cat] = str(path.resolve())

    # aggregated minute (365,1440)
    if EXPORT_MINUTE_LONG:
        agg_long = matrix_to_long_series(
            aggregated, freq="T", year=2020, col_name="power_W", add_datetime_index=True
        )
        agg_path = save_dataframe_csv(agg_long, PM.outputs_dir / "profile_aggregated_minute_long.csv")
    else:
...
def run_multi_archetype() -> Dict:
    ...
    for cat in global_categories:
        ...
        if EXPORT_MINUTE_LONG:
            df_long = matrix_to_long_series(
                mat, freq="T", year=2020, col_name="power_W", add_datetime_index=True
            )
            path = PM.outputs_dir / f"profile_{_safe_name(cat)}_minute_long.csv"
            save_dataframe_csv(df_long, path)
            per_user_csv[cat] = str(path.resolve())
    ...
    if EXPORT_MINUTE_LONG:
        agg_long = matrix_to_long_series(
            agg_mat, freq="T", year=2020, col_name="power_W", add_datetime_index=True
        )
        agg_path = save_dataframe_csv(agg_long, PM.outputs_dir / "profile_aggregated_minute_long.csv")
    else:
