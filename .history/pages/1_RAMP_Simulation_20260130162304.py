...
if agg_minute is None:
    ...
    hourly_long_df = matrix_to_long_series(
        hourly, freq="H", year=2020, col_name="power_W", add_datetime_index=True
    )
    ...
    if agg_long_path.exists():
        with open(agg_long_path, "rb") as f:
            st.download_button(
                "Download aggregated minute profile (LONG, 525600 rows)",
                data=f.read(),
    ...
    if out_path_long.exists():
        with open(out_path_long, "rb") as f:
            st.download_button(
                "Download aggregated hourly profile (LONG, 8760 rows)",
                data=f.read(),
